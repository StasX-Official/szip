  ...
  try {
    ...
    return new Promise((resolve) => {
      yauzl.open(archivePath, { lazyEntries: true }, (err, zipfile) => {
        ...
        let hasPasswordMetadata = false;
        
        zipfile.readEntry();
        zipfile.on('entry', (entry) => {
          if (entry.fileName === '.szip-metadata') {
            hasPasswordMetadata = true;
          }
          zipfile.readEntry();
        });
...
async function handleUnzip(parsed) {
  ...
  if (!password) {
    const needsPwd = await needsPassword(parsed.source);
    if (needsPwd) {
      try {
        const inquirerModule = await import('inquirer');
        ...
        Logger.error(401, 'Password input not available or password required but not provided');
        return;
      }
    }
  }
